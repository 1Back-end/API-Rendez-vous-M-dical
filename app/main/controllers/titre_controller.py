...
router = APIRouter(prefix="/titer", tags=["titre"])
...
async def create_titre(
    *,
    ...
    exist_titre_name = crud.titre.get_by_name(db=db, name=obj_in.name)
    if exist_titre_name:
        raise HTTPException(status_code=409,detail=__(key=",name-titre-already-exist"))
...
async def update_titre(
    *,
    ...
    exist_titre_name = crud.titre.get_by_name(db=db,email=obj_in.email)
    if exist_titre_name:
        raise HTTPException(status_code=409,detail=__(key="name-titre-already-exist"))
