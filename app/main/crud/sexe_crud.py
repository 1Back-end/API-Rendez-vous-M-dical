...
class CRUDSexe(CRUDBase[models.Sexe,schemas.SexeCreate,schemas.SexeUpdate]):
    ...
    def soft_delete(cls,db:Session,uuid:str):
        db_obj = cls.get_by_uuid(db=db,uuid=uuid) 
        if not db_obj:
            raise HTTPException(status_code=404,detail="sexe-not-found")
    ...
    def get_many(
        ...
        if order and order_field and hasattr(models.Sexe, order_field):
            if order == "asc":
                record_query = record_query.order_by(getattr(models.Sexe, order_field).asc())
            else:
